...
export class RealtimeFeeds {
  ...
  track(ws,venue,reconnect){
    ...
    ws.addEventListener('open',()=>{this.retries[venue]=0;this.setStatus(venue,'connected','等待首个行情事件');});
  ...
  connectBitget(){
    ...
    chunks(available,CHUNK).forEach((symbols)=>{
      const connect=()=>{ if(this.closed)return; const ws=new WebSocket(WS.bitget); this.track(ws,'bitget',connect);
        ws.onopen=()=>{this.retries.bitget=0;this.setStatus('bitget','connected','等待首个行情事件');ws.send(JSON.stringify({op:'subscribe',args:symbols.map(symbol=>({instType:'USDT-FUTURES',channel:'ticker',instId:symbol}))}));ws._ping=setInterval(()=>{if(ws.readyState===1)ws.send('ping');},25000);};
  ...
  connectGate(){
    ...
    chunks(available,80).forEach((contracts)=>{
      const connect=()=>{ if(this.closed)return; const ws=new WebSocket(WS.gate); this.track(ws,'gate',connect);
        ws.onopen=()=>{this.setStatus('gate','connected','等待首个行情事件');const time=Math.floor(Date.now()/1000);ws.send(JSON.stringify({time,channel:'futures.book_ticker',event:'subscribe',payload:contracts}));ws.send(JSON.stringify({time,channel:'futures.tickers',event:'subscribe',payload:contracts}));};
