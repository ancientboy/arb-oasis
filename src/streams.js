...
export class RealtimeFeeds {
  ...
  track(ws,venue,reconnect){
    ...
    ws.addEventListener('open',()=>this.setStatus(venue,'live'));
    ws.addEventListener('error',()=>this.setStatus(venue,'error'));
    ws.addEventListener('close',()=>{this.setStatus(venue,'reconnecting');if(!this.closed)setTimeout(reconnect,1800+Math.random()*1200);});
  }
  ...
  connectBitget(){
    ...
    chunks(available,CHUNK).forEach((symbols,idx)=>{
      const connect=()=>{ if(this.closed)return; const ws=new WebSocket(WS.bitget); this.track(ws,'bitget',connect);
        ws.onopen=()=>{this.setStatus('bitget','live');ws.send(JSON.stringify({op:'subscribe',args:symbols.map(symbol=>({instType:'usdt-futures',topic:'ticker',symbol}))}));ws._ping=setInterval(()=>{if(ws.readyState===1)ws.send('ping');},25000);};
        ws.onclose=()=>{clearInterval(ws._ping);this.setStatus('bitget','reconnecting');if(!this.closed)setTimeout(connect,1800+idx*40);};
        ws.onmessage=(ev)=>{if(ev.data==='pong')return;try{const msg=JSON.parse(ev.data);const symbol=msg.arg?.symbol;if(!symbol||!this.meta.bitget.has(symbol))return;for(const x of msg.data||[])this.merge('bitget',symbol,{bid:x.bid1Price,ask:x.ask1Price,bidQty:x.bid1Size,askQty:x.ask1Size,last:x.lastPrice,mark:x.markPrice,index:x.indexPrice,funding:x.fundingRate,nextFundingTime:x.nextFundingTime,openInterest:x.openInterest,volumeQuote:x.turnover24h,ts:x.ts||Date.now(),source:'WS'});}catch{}};
      }; connect();
    });
  }
  ...
  connectGate(){
    ...
    chunks(available,80).forEach((contracts,idx)=>{
      const connect=()=>{ if(this.closed)return; const ws=new WebSocket(WS.gate); this.track(ws,'gate',connect);
        ...
        ws.onclose=()=>{this.setStatus('gate','reconnecting');if(!this.closed)setTimeout(connect,1800+idx*40);};
